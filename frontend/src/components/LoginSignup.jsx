...
const LoginSignup = () => {
  ...
  const handleSignupClick = () => setShowAuthModal(true);
  const handleRoleSelection = (selectedRole) => {
    setRole(selectedRole);
    setIsVendorOrWholesalerModalOpen(true);
    setShowAuthModal(false);
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <h2 className="text-2xl md:text-3xl font-semibold text-center">Sign Up or Sign In</h2>
      <button
        onClick={handleSignupClick}
        className="bg-indigo-600 text-white px-4 py-2 md:px-6 md:py-3 rounded-md mt-4 mx-auto block hover:bg-indigo-700 transition duration-300 w-full max-w-xs"
      >
        Sign Up / Sign In
      </button>
      ...
      <Modal
        ...
        onRequestClose={() => setShowAuthModal(false)}
        style={customStyles}
        contentLabel="Choose Role"
      >
        <div className="space-y-4">
          <h2 className="text-xl md:text-2xl font-semibold text-center">Choose Your Role</h2>
          <button
            onClick={() => handleRoleSelection('vendor')}
            className="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 transition duration-300 w-full"
          >
            Vendor
          </button>
          <button
      ...
      <Modal
        ...
        style={{
          ...
          content: {
            ...
            maxWidth: '500px', // Slightly wider for forms
          },
        }}
      ...
      >
        <div className="space-y-3">
          ...
          <form onSubmit={handleRegister} className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
            ...
            {/* Other form fields (truncated for brevity) */}
            <input
              type="password"
              className="w-full p-2 border rounded-md"
              placeholder="Password"
